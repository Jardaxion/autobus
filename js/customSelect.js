...
//Function for show overflow or no
function onOverflow(overflow, selectBody){
    if(typeof $(selectBody).data('overflow') != "undefined"){
        overflow = $(selectBody).data('overflow');
    }

    if(overflow){
        var optionsHeight =  $(selectBody + ' .option').outerHeight() * 5;
        $(selectBody + ' .options').height(optionsHeight);
    }
}
